# -*- coding: utf-8 -*-
"""Day 22 骨架自检：mapper XML 良构 + statement 齐全 + SQL 填没填 + Java 占位计数。

（沿用 day19 / day20-xml-check.py 的配置化结构。Day 22 与 Day 20 的两处差别：
  1. 主角模块换成 **mall-admin / DashboardMapper**（本日唯一新建的 XML）；
  2. ★ Java 占位计数从「只扫 mall-marketing」改成**扫全部模块** ——
     因为本日的骨架落在 mall-user（AdminUserServiceImpl / UserServiceImpl）
     与 mall-admin（DashboardServiceImpl）**两个**模块，
     只扫 marketing 会得到「0 处占位」的假绿。

检查五类「编译期看不见」的错（判据同 day20，不再赘述）：
...
MODULES = [
    {
        "name": "mall-admin / DashboardMapper（★ Day 22 主角）",
        "java": ADMIN + r"\java\com\mallx\admin\mapper\DashboardMapper.java",
        "xml": ADMIN + r"\resources\mapper\DashboardMapper.xml",
        "new": ["countUsers", "countProducts", "countOrders", "sumPaidAmount",
                "selectDailyTrend"],
    },
    {
        "name": "mall-admin / AdminMapper（Day 07 回归）",
...
# ★ Day 22：扫全部模块（跳过后端 target 目录），基线 = mall-user 5 + mall-admin 2
TODO_SCAN_ROOT = ROOT
TODO_BASELINE = 7

REPORT = lp(r"D:\MallX\backend\loadtest\day22-xml-check-report.txt")
...
def count_java_todos(root_dir):
    ...
    ★ Day 22 起扫全部模块：骨架可能落在任意模块（本日就是 mall-user + mall-admin
      两处），只扫一个模块会得到「0 处占位」的假绿。跳过 target/ 避免扫到编译产物。
    """
    hits = []
