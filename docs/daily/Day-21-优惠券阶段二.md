...
>
> ✅ **状态：已实现并验收（2026-09-24 晚）** —— 6 处实现 + order 侧 2 根线全部落地。
> - `mvn -o install -DskipTests` → **12 / 12 BUILD SUCCESS**。
> - `day21-coupon-use-verify.py`（新建）→ **53 / 53** `VERDICT: OK 全绿`（A–G 七组）。
> - `day20-coupon-verify.py` → **82 / 82**（阶段一的 6 个端点一个都没被带坏）。
...
**★★ Day 21 验收实测结果（2026-09-24 晚 —— 不是计划，是跑出来的）**

| 项 | 结果 |
|---|---|
| `mvn -o install -DskipTests` | **12 / 12 BUILD SUCCESS** |
| `day21-coupon-use-verify.py`（新建，A–G 七组） | **53 / 53** `VERDICT: OK 全绿` |
| `day20-coupon-verify.py` | **82 / 82** |
| `day17-m1-regression.py` | **198 / 198** + `BASELINE RESTORED: YES` + `M1 REGRESSION: OK` |

★ 三条 E2E 的分项：Day 14 **67/67**（this run）/ Day 15 **56/56** / Day 16 **75/75**；
`orders=8 order_items=9 payments=5 reviews=0 inventory_logs=0 cart_items=0` 全部
`unchanged`、`inventories: identical to baseline`。

★★ **脚本自造夹具的原因**（写下来给以后抄）：库里 7 个真实 SKU 的价格全在 6499–12499，
而本日要断言的边界是**封顶**（面额 300 > 总额 200）、**小数位**（33.45 × 0.10 = 3.345）、
**门槛差一分**（99.99 < 100）—— 拿几千块的商品**造不出这些总额**。
所以脚本自造 1 个临时商品 + **4 个价格受控的 SKU**（100.00 / 200.00 / 99.99 / 33.45）
+ 库存 1000 件（**全程不碰真实 SKU 的库存行**，M1 的逐 SKU 指纹才不会被搅动）。
★ 边界券同样**SQL 直造**：`rate=80` 的脏券按定义过不了接口（决策 A 已把 DTO 收到 1.00），
而已过期 / 尚未开始 / 已下架的券**根本领不到**（领券 CAS 守窗口两端）——
要验这两类，只能绕过接口写库。

★★ **本轮最值钱的两条**

1. **`cleanup()` 必须有「水位线未初始化就拒绝执行」的护栏**。订单族靠高水位线删
   （`DELETE FROM orders WHERE id > HWM`），而 `HWM` 初值若是 `0`，这一句就是
   **把这台机器上全部历史订单删掉**（本项目共 8 条，删了 M1 立刻红）。
   本脚本第一版就把 `cleanup()` 写在了「读水位线」之前 —— 写完当场自查发现并改序。
   ⇒ 与 Day 18「骨架脚本误跑真写库」同类：**护栏要判「前提是否成立」，不是「值看起来像不像 0」**。
2. **HALF_UP 的断言要挑「能区分」的数**：`33.45 × 0.10 = 3.345` —— HALF_UP 给 **3.35**、
   HALF_EVEN 给 **3.34**。若挑 `33.35 × 0.10 = 3.335`（3.33 与 3.34 之间），
   两种舍入模式**给同一个答案**，脚本看不出代码是不是写错了舍入模式。
   ⇒ **凡是断「舍入/边界」，先确认这个样本能让两种实现给出不同结果**（同 L1 的样本量陷阱）。
