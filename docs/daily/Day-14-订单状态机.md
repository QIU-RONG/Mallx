...
1. **端到端链路**：下单 → 取消 → 查库对账 → 再下单 → 支付 → 对账（两条出路各走一次）
   → ✅ **已补做**：`backend/loadtest/day14-e2e-walk.py`，**67/67**，见 §十三
...
## 十三、第 5 步实测（补）：端到端两条出路 A/B —— 67/67 全绿

**脚本**：`backend/loadtest/day14-e2e-walk.py`（报告 `day14-e2e-walk-report.txt`）

§十二 是**库级**对账（审计全库现值 vs 账本）；本节是**接口级**的 A/B 走查 ——
把「下单 → 取消」与「下单 → 支付」两条出路，放在**同一条未变动的基线**上先后各走一遍。

### 13.1 为什么要单列一节（§七 第 1 条的欠账）

§十一 里 `place→pay` 与 `place→cancel→release` 是**两条独立的腿**（各自跑各自的用例）；
而 §七 第 1 条要的是「两条出路各走一次」的**对照**。这两件事不是一回事：
前者的结论是「每条腿单独看没问题」，后者的结论是
「**同一个起点分岔后，两条腿只在一个点上不同**」—— 后者才真正证明了「状态机」这三个字。
所以补了这一节，把欠账还掉。

### 13.2 ★★ 对照表（本节的核心产出）

| 步骤 | `available` | `locked` | `sold` | 订单状态 |
|---|---|---|---|---|
| start | 147 | 0 | 3 | — |
...
读法：`A/placed` 与 `B/placed` **逐字段完全相同** —— 两条路走到分岔口为止一模一样；
差异**只**出现在最后一条边：

| 最后一条边 | `available` | `locked` | `sold` | 语义 |
|---|---|---|---|---|
| `PENDING_PAYMENT → CANCELLED` | **+1** | −1 | 0 | 货**回到货架** |
| `PENDING_PAYMENT → PAID` | **0** | −1 | **+1** | 货**永远离开** |

`locked` 在两条路上都回到 0 —— **锁一定会被解掉**，区别只在解到哪个格子。

### 13.3 账本链（4 行，无缝）

```
id   type              change  before  after  ref
19   ORDER_LOCK            -1     147    146  NULL
20   CANCEL_RELEASE        +1     146    147  9025
21   ORDER_LOCK            -1     147    146  NULL
22   PAY_SOLD              +0     146    146  9026
```

`147 → 146 → 147 → 146 → 146`：`row[i].before == row[i-1].after`，**无一处断裂**。
两条出路各留「自己的两行」，`PAY_SOLD` 的 `change` 恒为 0（`available` 没被支付碰过）。
对账：`A0 + Σ(change) = 147 + (−1) = 146 = available now` ✅。

### 13.4 清理与可重跑

脚本**自带清理**（放在 `finally` 里，断言失败也照样执行）：删本轮订单 + 关联行 + 本轮流水，
再把 sku4 倒回基线。实测**连跑两次都是 67/67**，第二次自动取新订单号（#9027 / #9028）并再次自清 ——
**不留痕、可重跑**。清完 7 个 SKU 逐字段等于 §0.3 基线，行数回到 `8/5/9/0`，孤儿行 0。

★ 与 §十二 的分工：§十二 用独立的 `day14-reconcile.py` + `day14-step5-cleanup.sql` 做全库审计与收官；
本节是**自包含单脚本**走查，随时可重跑、不需要配套 SQL。

---

**状态**：✅ 第 1 步（取消订单，34/34）+ ✅ 第 2 步（并发对照，主组 16/16、对照组 11/11）
+ ✅ 第 3 步（超时关单，22/22）+ ✅ 第 4 步（`inventory_logs` 流水，61/61；前置 `RETURNING` 探针 17/17）
+ ✅ 第 5 步（流水对账 28/28 + 收官清理 + 端到端两条出路 A/B 67/67）全部完成并提交。
**Day 14 收官**：库存三个写点全部留痕，账本可反向审计现值，两条出路已做同基线 A/B 对照。
